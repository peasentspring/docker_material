...
class abtest(object):
    ...
    #===========================================================================
    def abtest(self, filename):
        p = subprocess.Popen("ab -c 100 -n %d %s"%(self.ab_n, self.host_path), stdout=subprocess.PIPE, shell=True)
        self.result = p.communicate()
        fh = open(os.path.join('.', filename), 'w+')
        fh.write(self.result[0])
        fh.close()
...
#         return m.group(1)
    ...
    def get_transfer_rate(self):
        #print self.result
        regxobj = re.compile("Transfer rate:\s*(\d+\.\d+)")
        m = regxobj.search(self.result[0])
        return m.group(1)    
